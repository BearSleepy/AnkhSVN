...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="AnhkBinDir" select="'Debug'" />
  <xsl:param name="BitmapFile" select="'../References/CtCBitmap.bmp'" />
  ...
  Type LoadType(string from, string type, string sourceFile,
      string configuration, string AnhkBinDir)
  {
    if( from.Contains("$(Configuration)") ||
        from.Contains("$(AnhkBinDir)"))
    {
      if(string.IsNullOrEmpty(configuration) ||
          string.IsNullOrEmpty(AnhkBinDir))
        throw new InvalidOperationException("Configuration not set");

      from = from
        .Replace("$(Configuration)", configuration)
        .Replace("$(AnhkBinDir)", AnhkBinDir);;
    }
  ...
  public string InputType(string type, string from, string prefix, string sourceFile,
      string configuration, string AnhkBinDir)
  {
    StringBuilder sb = new StringBuilder();

    Type tp = LoadType(from, type, sourceFile, configuration, AnhkBinDir);
  ...
  public XPathNodeIterator InputSymbol(string type, string from, string prefix, string sourceFile,
      string configuration, string AnhkBinDir)
  {
  ...
  public string generateBitmap_NoFile(XPathNodeIterator nodes, string bitmapFile, string sourceFile, string imgSubdir)
  {
    string baseDir = Path.GetDirectoryName(sourceFile);
    StringBuilder sb = new StringBuilder();
    int imgCount = nodes.Count;
  ...
  public string generateStrip(string bitmap, string type, string assembly, XPathNodeIterator keys,
      XPathNodeIterator images, bool transparent, string sourceFile,
      string configuration, string AnhkBinDir)
  {
    string baseDir = Path.GetDirectoryName(sourceFile);
    Type tp = LoadType(assembly, type, sourceFile, configuration, AnhkBinDir);
