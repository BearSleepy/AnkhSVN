	...
	sealed class AddToSccCommands : CommandBase
	{
		...
		static bool HandleUnmanagedOrUnversionedSolution(CommandEventArgs e, SvnItem solutionItem)
		{
			...
			if (!confirmed && !e.DontPrompt && !e.IsInAutomation &&
				DialogResult.Yes != mb.Show(string.Format(Resources.MarkXAsManaged,
		...
		static bool AddVersionableSolution(CommandEventArgs e, SvnItem solutionItem, ref bool confirmed)
		{
			...
			if (!e.DontPrompt && !e.IsInAutomation)
			{
				...
				DialogResult rslt = mb.Show(string.Format(Resources.AddXToExistingWcY,
		...
		static bool CheckoutWorkingCopyForSolution(CommandEventArgs e, ref bool confirmed)
		{
			...
			using (AddToSubversion dialog = new AddToSubversion())
			{
				...
				if (dialog.CommitAllFiles)
				{
					...
					{
						dlg.Text = Resources.ImportingTitle;
					...
					{
						e.GetService<IProgressRunner>().RunModal(Resources.ImportingTitle,
		...
		static bool AskSetManagedSelectionProjects(CommandEventArgs e, IProjectFileMapper mapper, IAnkhSccService scc, IEnumerable<SccProject> succeededProjects)
		{
			...
			if (li > 0)
				txt = txt.Substring(0, li + 1) + Resources.FileAnd + txt.Substring(li + 3);

			return DialogResult.Yes == mb.Show(string.Format(Resources.MarkXAsManaged,
