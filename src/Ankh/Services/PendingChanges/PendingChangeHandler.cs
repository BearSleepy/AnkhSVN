...
namespace Ankh.Services.PendingChanges
{
	...
	partial class PendingChangeHandler : AnkhService, IPendingChangeHandler
	{
		...
		public bool CreatePatch(IEnumerable<PendingChange> changes, PendingChangeCreatePatchArgs args)
		{
			...
			{
				GetService<IProgressRunner>().RunModal(Resources.DiffTitle,
		...
		private bool PreCommit_VerifyNoConflicts(PendingCommitState state)
		{
			foreach (PendingChange pc in state.Changes)
			{
				...
				{
					state.MessageBox.Show(Resources.OneOrMoreItemsConflicted,
						"",
			...
			foreach (PendingChange pc in state.Changes)
			{
				...
				if (item.IsVersioned || item.IsVersionable)
				{
					...
					else if (w != null && w != wc && w.RepositoryRoot != wc.RepositoryRoot)
					{
						...
						sb.AppendLine(Resources.CommitSingleWc);
						sb.AppendFormat(Resources.WorkingCopyXFromRepositoryY, wc.FullPath, wc.RepositoryRoot);
						sb.AppendLine();
						sb.AppendFormat(Resources.WorkingCopyXFromRepositoryY, w.FullPath, wc.RepositoryRoot);
						sb.AppendLine();
		...
		private bool PreCommit_VerifyLogMessage(PendingCommitState state)
		{
			...
			if (CommitSettings.WarnIfNoIssue && !state.SkipIssueVerify)
			{
				...
				if (!haveIssue &&
					state.MessageBox.Show(Resources.NoIssueNumber, "",
		...
		private bool PreCommit_SaveDirty(PendingCommitState state)
		{
			...
			{
				state.MessageBox.Show(Resources.FailedToSaveBeforeCommit, "", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Exclamation);
				return false;
			}
		...
		private bool PreCommit_AddNewFiles(PendingCommitState state)
		{
			...
			while (toAdd.Count > 0)
			{
				...
				state.GetService<IProgressRunner>().RunModal(Resources.AddingTitle,
					delegate(object sender, ProgressWorkerArgs e)
					{
				...
				if (error != null)
				{
					if (error.SvnErrorCode == SvnErrorCode.SVN_ERR_WC_UNSUPPORTED_FORMAT)
					{
						state.MessageBox.Show(error.Message + Environment.NewLine + Environment.NewLine
							+ Resources.YouCanDownloadAnkh, "", MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Error);
						return false;
					}
		...
		private bool PreCommit_VerifyTargetsVersioned(PendingCommitState state)
		{
			...
			foreach (PendingChange pc in state.Changes)
			{
				...
				{
					state.MessageBox.Show(string.Format(Resources.CantCommitUnversionedTargetX, pc.FullPath), "", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Error);
					return false;
				}
			}
			return true;
		...
		private bool Commit_CommitToRepository(PendingCommitState state)
		{
			...
			state.GetService<IProgressRunner>().RunModal(Resources.CommitTitle,
				delegate(object sender, ProgressWorkerArgs e)
				{
					...
					if (!ok && ca.LastException != null)
					{
						...
						if (!string.IsNullOrEmpty(itemPath))
						{
							...
							outOfDateMessage.AppendFormat(Resources.WhileCommittingX, itemPath);
						}
					}
				});
			...
			if (outOfDateMessage != null)
			{
				state.MessageBox.Show(outOfDateMessage.ToString(),
									  outOfDateError ? Resources.OutOfDateCaption : Resources.CommitFailedCaption,
			...
			if (rslt != null)
			{
				...
				if (ci != null)
					ci.OnLastChanged(new LastChangedEventArgs(Resources.CommittedPrefix, rslt.Revision.ToString()));

				if (!string.IsNullOrEmpty(rslt.PostCommitError))
					state.MessageBox.Show(rslt.PostCommitError, Resources.PostCommitError, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
