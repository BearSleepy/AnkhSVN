...
namespace Ankh.VS.SolutionExplorer
{
	...
	sealed class StatusImageMapper : AnkhService, IStatusImageMapper
	{
		...
		public ImageList CreateStatusImageList(bool width8)
		{
			...
			var bitmap = width8 ? Resources.StatusGlyphs8 : Resources.StatusGlyphs7;
			if (bitmap == null)
				return null;
			...
			return imageList;
		   //int width = width8 ? 8 : 7;
		   // using (Stream images = typeof(StatusImageMapper).Assembly
		   //         .GetManifestResourceStream(typeof(StatusImageMapper).Namespace
		   //         + string.Format(CultureInfo.InvariantCulture, ".StatusGlyphs{0}.bmp", width)))
		   // {
		   //     if (images == null)
		   //         return null;
