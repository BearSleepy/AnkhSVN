	...
	sealed class AnnotateMarginControl : Control
	{
		...
		void ShowTip(AnnotateRegion region)
		{
			...
			if (rect.Contains(mp))
			{
				...
				if (src.Revision >= 0)
				{
					sb.AppendFormat(Resources.RevisonHead, src.Revision);
					sb.AppendLine();
					sb.AppendFormat(Resources.AuthorHead, src.Author);
					sb.AppendLine();
					sb.AppendFormat(Resources.TimeHead, src.Time.ToLocalTime());
					...
					sb.Append(Resources.LocalChange);
